...
static void XMLCALL sheet_start(void *ud, const char *name, const char **attrs) {
    ...
    case SS_NONE:
        ...
        /* Phase 15: sheet protection */
            ...
            const char *pwd_s    = attr(attrs, "password");  /* legacy MD4 hash */

            /* Boolean attribute helper: "1" or "true" = true, absent or "0" = false */
            #define BOOL_ATTR(s) ((s) && ((s)[0] == '1' || strcmp((s), "true") == 0))
            p->sheet               = BOOL_ATTR(sheet_s);
            p->objects             = BOOL_ATTR(obj_s);
            p->scenarios           = BOOL_ATTR(scen_s);
            /* Note: formatCells="1" in OOXML means "formatCells is LOCKED" (not allowed).
               We invert: our field = 1 means "allowed". Default (absent) = not allowed = 0. */
